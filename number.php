<?php
    ...
    $message = "";
    								...
    								if(isset($_POST['btnAdd'])){
    									$number = mysql_real_escape_string(trim(strtoupper($_POST['r_number'])), $dbc);
    									$format_id = mysql_real_escape_string(trim($_POST['r_format']), $dbc);
    									$prefix = mysql_real_escape_string(trim($_POST['r_format']), $dbc);

    									$query1 = "SELECT * FROM number WHERE number = '$number'";
								     	$data1 = mysql_query($query1, $dbc)
								     	or die("Error selecting from database. " . mysql_error());

								     	$query2 = "SELECT * FROM format WHERE format_id = '$format_id'";
    									...
    									$plateNumber = $formatP . '-' . $number;
								     	if(mysql_num_rows($data1) == 0){
	    									$query1 = "INSERT INTO number(format_id,number)VALUES ('$format_id','$plateNumber')";
	    									$data1 = mysql_query($query1, $dbc)
...
<html>
<head>
	<title>Plate Number | Automated License Plate Recognition</title>
							...
							<select name="r_format">
								<?php
						            ...
						            while ($row = mysql_fetch_array($data)) {
						            	$prefix = $row['prefix'];
						            	echo '<option value="'.$row['format_id'].'">' . $row['prefix']. '</option>';
						            }
								 ?>		
							</select>&nbsp;
